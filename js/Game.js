...
MP.Game.prototype = {
    ...
    create: function() {
        ...
        this.add.sprite(0, 0, 'background');
		  ...
		  this.cloud = this.add.tileSprite(0, 0, this.world.width, this.world.height, 'cloud');
	...
	var sprite = this.add.sprite(470, 23, 'chaos');
  
	
	this.counterDisplay = this.add.text(this.world.centerX+220, this.game.world.centerY-228, '' + this.counter);


    //	Center align
    this.counterDisplay.anchor.set(0.5,0.5);
    this.counterDisplay.align = 'center';

    //	Font style
    this.counterDisplay.font = 'Montserrat';
	this.counterDisplay.fill = '#ffffff';
    this.counterDisplay.fontSize = 24;
     this.counterDisplay.fontWeight = 'bold';

	  this.counterDisplay.stroke = '#ffffff';
    this.counterDisplay.strokeThickness = 1;
		...
		//var powerup = this.add.sprite(0, 0, 'powerup');
		//this.events.loop(2000, function() {  
	//	this.add.tween(powerup).to({x: this.game.world.randomX, y: this.game.world.randomY}, 1750, Phaser.Easing.Quadratic.InOut, true);}, this)


		
        //t_score = this.add.text(12, 12, 'SCORE: ' + score.toString(), { font: '24px ', display: 'none' + t_font, fill: '#ffffff' });
	...
	updateCounter: function () {
        ...
        var circleTimer = this.game.add.graphics(this.game.world.centerX, this.game.world.centerY);
       
		circleTimer.lineStyle(12, 0xFFFFFF);
	  circleTimer.smooth = true;
	  
        var sa = this.game.math.degToRad(-90+(360/this.counterMax)*(this.counterMax-(this.counter+1)));
        var ea = this.game.math.degToRad(-90+(360/this.counterMax)*(this.counterMax-this.counter));
        
	var arc = circleTimer.arc(220, -230, 30, sa, ea, false);
    ...
    update: function() {
	 ...
	 this.cloud.tilePosition.x += this.time.physicsElapsed * SPEED / 2;
	 this.cloud.tilePosition.y -= this.time.physicsElapsed * SPEED / 2;
    ...
    spawnBalloon: function() {
        ...
        // Position
        b.y = 640;
        b.x = this.randomInt(0, 550 - b.width);
        // Value (number)
        ...
        b.numval = numval;
        bt = this.add.text(b.x, b.y + 60, numval.toString(), { font: '0.1px ' + t_font, fill: '#000' });
